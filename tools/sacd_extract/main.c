...
static int parse_options(int argc, char *argv[]) 
{
    ...
    if (optind < argc) {
        ...
        strncpy(opts.output_file, remaining_arg, sizeof(opts.output_file) - 1);
        opts.output_file[sizeof(opts.output_file) - 1] = '\0';
    }
        ...
        if (!nogo) 
        {
            ...
            if (handle)
            {
                ...
                if (opts.output_dsf || opts.output_iso || opts.output_dsdiff || opts.output_dsdiff_em || opts.export_cue_sheet)
                {
                    ...
                    else if(opts.multi_channel && !has_multi_channel(handle)){
                        fprintf(stderr, "Asked multichannel format but disc has no multichannel area\n");
                    }

                    if(n_areas == 0){
                        fprintf(stderr, "No channel areas selected for output\n");
                        nogo = 1;
                    }
...
#define FAT32_SECTOR_LIMIT 2090000
...
#endif
                        {
                            ...
                            iso_path = get_unique_path(opts.output_dir, albumdir_loc, "iso");
                            scarletbook_output_enqueue_raw_sectors(output, 0, total_sectors, iso_path, "iso");
                    ...
                    else if (opts.output_dsdiff_em)
                    {
                        ...
                        for(j = 0; j < n_areas; j ++){
                            albumdir_loc = (char *)malloc(strlen(albumdir) + 10);
                            strcpy(albumdir_loc, albumdir);
                            if(n_areas > 1){
                                strcat(albumdir_loc, j ? " [multi]" : " [stereo]");
                            }
                    ...
                    if(opts.export_cue_sheet){
                        ...
                        for(j = 0; j < n_areas; j ++){
                            ...
                            const char *iso_name = strrchr(opts.input_device, '/');
                            char *cue_ref;
                            if (!iso_name) iso_name = strrchr(opts.input_device, '\\');
                            cue_ref = strdup(iso_name ? iso_name + 1 : opts.input_device);
                            ...
                            /* CUE at output_dir/AlbumName.cue when -I or -e (single file), or inside album subdir with audio */
                            if(opts.output_iso || opts.output_dsdiff_em){
                                file_path = make_filename(opts.output_dir, 0, albumdir_loc, "cue");
                            } else {
                                char *cue_dir = make_filename(opts.concurrent ? opts.output_dir_conc : opts.output_dir, albumdir_loc, 0, 0);
                                MKDIR(cue_dir, 0774);
                                file_path = make_filename(cue_dir, 0, albumdir_loc, "cue");
                                free(cue_dir);
                            }

                            CHAR2WCHAR(wide_filename, file_path);
                            safe_fwprintf(stdout, L"\nExporting CUE sheet [%ls]\n", wide_filename);
                            write_cue_sheet(handle, cue_ref, area_idx[j], file_path);
                            free(file_path);
                            free(wide_filename);
                            free(cue_ref);
                            free(albumdir_loc);
